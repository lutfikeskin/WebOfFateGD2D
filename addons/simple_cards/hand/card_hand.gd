...
func _process(_delta: float) -> void:
	...
	if _dragged_card and (not is_instance_valid(_dragged_card) or _dragged_card.is_queued_for_deletion()):
		_dragged_card = null
...
func _arrange_cards() -> void:
	# Cleanup invalid cards first
	for i in range(_cards.size() - 1, -1, -1):
		if not is_instance_valid(_cards[i]) or _cards[i].is_queued_for_deletion():
			_cards.remove_at(i)
...
func _arrange_line(skip_dragged: bool = false) -> void:
	...
	if not is_instance_valid(_cards[0]): return
